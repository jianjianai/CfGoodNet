...
export const REJECToutbound: Outbound = {
  handleRequest(_clientReq: IncomingMessage, clientRes: ServerResponse, _targetUrl: URL) {
    // 直接拒绝请求
    clientRes.writeHead(403, { "Content-Type": "text/plain" });
    clientRes.end("Blocked by proxy rule");
  },

  handleUpgrade(_clientReq: IncomingMessage, clientSocket: Socket, _head: Buffer, _targetUrl: URL) {
    // WebSocket 升级时同样拒绝
